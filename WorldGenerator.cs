...
public class WorldGenerator : MonoBehaviour
{
    ...
    void DynamicCreateChunck()
    {
        ...
        foreach (var player in players)
        {
            ...
            // В начало метода — получить фрустум раз/фрейм
            var cam = mainCam;// или твоя камера
            var camPos = cam.transform.position;
            var camForward = cam.transform.forward;
            Plane[] frustumPlanes = GeometryUtility.CalculateFrustumPlanes(cam);

            // предвычисляем cos половинного максимального угла (учтём и горизонтальный FOV)
            ...
            // ВАЖНО: лучше работать в координатах чанков (целых)
            // size — размер чанка (int)
            int chunkSize = size;
                ...
                for (int cy = minChunkY; cy <= maxChunkY; cy++)
                {
                    for (int cz = minChunkZ; cz <= maxChunkZ; cz++)
                    {
                        ...
                        if (dot < cosHalfMax)
                        {
                            ...
                            continue; // слишком в сторону — не видим
                        }

                        // 3) Точный тест фрустума (AABB)
    ...
    public ChunckComponent CreateChunck(int posX, int posY, int posZ)
    {
        ...
        {
            //int iii = 0;
            //var str = $"Позиция X:{posX} Y:{posY} Z:{posZ} \nБлоки:[";
