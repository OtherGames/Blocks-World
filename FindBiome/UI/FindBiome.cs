...
public class FindBiome : MonoBehaviour
{
    ...
    private void BtnFind_Clicked()
    {
        ...
        BiomeGenerationData[] ebala = new BiomeGenerationData[WorldGenerator.Inst.procedural.biomes.Length];
        ...
        IEnumerator Async()
        {
            ...
            if (int.TryParse(inputos.text, out var targetBiomeIdx))
            {
                while (true)
                {
                    var biomeIdx = generation.GetBiomeIndex(startX, startY, startZ, out var biome);
